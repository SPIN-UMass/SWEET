...
#define PORT "3129"		// port we're listening on
...
//    }
    ...
    if (p == NULL) {
	fprintf(stderr, "proxy 2 client: failed to connect\n");
	exit(2);
    }
...
//                      perror("infile open");
		    ...
		    //char data[1500];
		    //int bytes = read(file_fd, data, 1500);
		    //printf("data: %s\n", data);
		    //for(j = 0; j < bytes; j++)
		    //printf("%x ", data[j]);
		    //printf("\n");
...
*/
			sprintf(cmd,
				"sendemail -f mailmynet@hatswitch.crhc.illinois.edu -t netarch.emma@gmail.com -m 'oo' -u 'RE: www.google.com' -a %s",
				filename);
			system(cmd);
